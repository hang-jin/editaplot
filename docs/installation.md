...
- 用户自行合法安装并激活的 Origin/OriginPro；测试基线为 2024b / 10.15；
- 绘图前由用户亲自确认 Origin 可以手动正常启动；
...
> Python；Origin 则永远不会被自动安装、启动、修改、破解或激活。
...
项目环境。不要安装、修改、激活或破解 Origin。完成后运行 editaplot.cmd doctor，并用中文告诉我
哪些项目已就绪、哪些仍需要我手动处理。
```
...
- `ready_for_render`：分析依赖和本机 Origin 应用都已检测到，但绘图前仍需用户确认合法许可与手动启动；
...
Origin 未安装、许可证无效或不能手动启动都不能由自动修复绕过。
...
宣称 Origin 已就绪。Origin 安装、授权、激活与手动启动始终由用户负责。
...
CPython 3.10–3.12 and a separately obtained, legally licensed Origin/OriginPro installation that
you can start manually. CLI/dependency coverage spans all three Python minors; the current live
