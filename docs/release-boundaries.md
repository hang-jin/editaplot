...
- 只连接用户电脑上已经存在且可供 Automation 调用的 Origin/OriginPro；不分发 Origin 应用。
- 不把 Origin Automation Server 暴露到公共网络，不提供云端绘图或 service-bureau 服务。
- “兼容 Origin”只描述互操作性，不表示 OriginLab 认可、赞助或提供本项目。
- 环境自动修复只安装项目级 Python 依赖，不安装或修改 Origin；render 仅在用户请求绘图时连接本机 Automation 应用。

OriginLab 的 [External Python 文档](https://docs.originlab.com/externalpython/)说明了本机 Windows、Origin 版本和 Automation 的技术要求。EditaPlot 的端到端验证基线是 Origin/OriginPro 2024b（10.15）；其他版本只有在生成完整可编辑产物、对象反读和视觉 QA 后，才会标记为已验证。
