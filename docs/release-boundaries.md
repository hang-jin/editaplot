...
公开仓库包含完整 Codex Skill、清理后的自包含 runtime、模板、确定性 CLI、测试、双语文档、中性合成示例、原创配色和 44 个经 Origin 生成并复核的 PNG。它不是功能受限版，也没有隐藏的“私人高级模板”。

我把“模板已实现”和“图片已进入公开 gallery”作为两件事管理。新增模板可以随源码发布，
但示例图只有完成 Origin 全格式产物、对象反读、人工视觉检查、脱敏和 provenance 审计后，
才会改变公开的 44 张计数；没有通过的预览不会进入首页或资产清单。
...
- XPS 多谱线对比、FTIR/IR、NMR、DSC、PL 多条件、UV–Vis 多样品和 40×40 高密度
  热力图已在 2024b 基线上生成 OPJU/PNG/PDF/TIF，并通过程序化对象反读、绑定 PNG
  哈希的人工视觉 QA 与公开 PNG 脱敏审计。
- 我不会为了增加案例数量而跳过门禁，也不会把本地绝对路径、开发日志、用户数据或
  未清理的中间图复制进公开仓库。
...
我不会把“检测到安装”“可以启动”或“Automation 握手成功”单独写成版本兼容成功。对
