...
你明确选择的精确系列颜色优先于从参考图中推测出的颜色。但 XPS component、正负效应、连续
热力图、诊断参考线、混淆矩阵等路线的颜色携带数据含义，不能被参考图或普通 cosmetic palette
静默替换。若你明确要求新色，我会先固定“哪条数据对应哪个颜色”，并检查该模板是否已经通过
Origin 写入、对象反读和视觉验证；通过才会标记为已采用，否则会清楚写成保留默认或拒绝，而不是
绕过能力门禁。
