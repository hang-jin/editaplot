...
`USERPROFILE`，也不一定拥有当前登录用户启动 Origin 的权限，所以 EditaPlot 会读取当前进程
真实的 Windows 安全令牌，而不是相信环境变量。检测到 Codex 沙箱时，它会在调用 Origin COM
之前停止，并让 Codex 只针对这一条 `origin-smoke` 或 `render` 命令发起正式、受限的本地执行
审批；你允许后，Codex 会重新执行同一条 Origin 命令并继续当前任务。你不需要把命令复制到自己
的 PowerShell，也不需要管理员权限、DCOM/注册表修改或所谓“绕过沙箱”。如果你所在组织或本机
策略拒绝这次审批，任务会清楚停止，不会把失败包装成 Origin 已完成。Codex 的自动审查功能即使
已经启用，也只是替你评估这一次请求，不等于提前赋予所有 Origin 命令权限。
